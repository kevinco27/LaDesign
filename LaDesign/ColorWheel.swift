...
class ColorWheel: UIView {
    ...
    var inner_radius:CGFloat!
    let inner_outer_ratio:CGFloat = 0.9
    ...
    init(frame: CGRect, color: UIColor!, nonTouchedRadius: CGFloat!) {
        ...
        inner_radius = radius * inner_outer_ratio
    ...
    func onTheTouchableWheel(_ coord: CGPoint, _ nonTouchedRadius: CGFloat) -> (Bool){
        ...
        if (distance < radius) && (distance > nonTouchedRadius){
            isOntheWheel = true
        }else{
            isOntheWheel = false
        }
    ...
    func getIndicatorCoordinate(_ coord: CGPoint) -> (point: CGPoint, isCenter: Bool) {
        ...
        // If the touch coordinate is inside the inner radius of the wheel, transform it to the inner edge of the wheel with polar coordinates
        if (distance < inner_radius){
            let theta: CGFloat = atan2(dy, dx)
            outputCoord.x = inner_radius * cos(theta) + wheelLayerCenter.x
            outputCoord.y = inner_radius * sin(theta) + wheelLayerCenter.y
        }
...
//        }
    ...
    func hueSaturationAtPoint(_ position: CGPoint) -> (hue: CGFloat, saturation: CGFloat) {
        ...
        if (saturation < 0.99 && saturation > inner_outer_ratio) {
            saturation = 0.98
        }else{
            saturation = 1
        }
