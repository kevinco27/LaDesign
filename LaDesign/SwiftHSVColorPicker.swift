...
open class SwiftHSVColorPicker: UIView, ColorWheelDelegate  {
    ...
    func setup() {
        ...
        colorWheel = ColorWheel(frame: CGRect(x: centeredX, y: (self.bounds.height - colorWheelSize)/2, width: colorWheelSize, height: colorWheelSize), color: self.color, nonTouchedRadius: selectedColorViewHeight/2)
        colorWheel.delegate = self
        // Add colorWheel as a subview of this view
        self.addSubview(colorWheel)
        
        // Init SelectedColorView subview
        selectedColorView = SelectedColorView(frame: CGRect(x: (self.bounds.width-selectedColorViewHeight)/2, y:(self.bounds.height-selectedColorViewHeight)/2, width: selectedColorViewHeight, height: selectedColorViewHeight), color: self.color)
